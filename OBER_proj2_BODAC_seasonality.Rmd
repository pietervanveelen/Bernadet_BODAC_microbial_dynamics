...
ggplot(aes(x=date, y=Chao1)) +
  ...
  geom_point(aes(color = year, fill = year, shape = sampling_time), size = 2, show.legend = T) + # position = position_jitterdodge(0.2)) +
  geom_line(aes(linetype = sampling_time, color = year)) +
  scale_color_manual(values = c(brewer.pal(6, "Blues")[3:6])) +
  scale_fill_manual(values = c(brewer.pal(6, "Blues")[3:6])) +
...
p = 
  ...
  facet_grid(cols=vars(filter_type), rows = vars(sampling_time)) +
  theme_classic()
...
```{r plot top ASVs granules, eval=T, echo=F, fig.height=3, fig.width=5, message=FALSE, warning=FALSE, cache=FALSE, include=T}
...
print(top_asv_rel_abund_granules)
