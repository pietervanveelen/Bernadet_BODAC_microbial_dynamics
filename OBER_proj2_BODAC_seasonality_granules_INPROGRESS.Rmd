...
psdata_OBER <- subset_samples(psdata_OBER, sample_type != "activated_sludge" & sample_type != "blank" & sample_type == "granule") # subset only BODAC samples granules (n=35)
psdata_OBER <- prune_taxa(taxa_sums(psdata_OBER) > 0, psdata_OBER) # 4612 taxa found in 35 BODAC granule samples



# average and variation in coverage
mean(sample_sums(psdata_OBER)) # = 48159 reads
summary(sample_sums(psdata_OBER))
sum(sample_sums(psdata_OBER)) # 1685576 reads after filtering BODAC granule samples

# number of reads in Blank (#538)
summary(sample_sums(psdata_OBER_blank)) # across 3 NCs 13565 reads were detected
...
ntaxa(psdata_OBER_0.01pct)  #3409 
ntaxa(psdata_OBER_0.05pct)  #2085 
ntaxa(psdata_OBER_0.1pct)   #1581 

# associated relative abundances with filters
num(100*(sum(sample_sums(psdata_OBER_0.01pct))/total_sum), digits = 3) # (99.83% abundance)
num(100*(sum(sample_sums(psdata_OBER_0.05pct))/total_sum), digits = 3) # (98.88% abundance)
num(100*(sum(sample_sums(psdata_OBER_0.1pct))/total_sum), digits = 3)  # (97.89% abundance)
...
sample_data(psdata_OBER)$combined = factor(paste0(
  sample_data(psdata_OBER)$filter_type, "-", 
  sample_data(psdata_OBER)$sample_type, "-", 
  sample_data(psdata_OBER)$sampling_time))

# show plot
#amp_rarecurve(psdata_OBER_unfiltered, color = "combined", legend.position = "bottomright")

# save plot
pdf("figures/OBER_proj2_BODAC_rarefaction_curves_granules.pdf", useDingbats = F, width = 6, height = 4)
amp_rarecurve(psdata_OBER_unfiltered, color = "combined", legend.position = "bottomright")
dev.off()
...
# difference in sample coverage (6.01)
...
ggplot(aes(x=date, y=Chao1)) +
  geom_point(aes(color = sampling_time, fill = sampling_time, group = sampling_time), 
             size = 2, show.legend = T) + 
  geom_line(aes(group = sampling_time, color = sampling_time)) +
  scale_color_manual(values = c("#CC79A7", "dodgerblue")) +
  scale_fill_manual(values = c("#CC79A7", "dodgerblue")) +
  geom_vline(xintercept = as_date("2020-01-01"), linetype = "dashed", color = "grey50") +
  geom_vline(xintercept = as_date("2021-01-01"), linetype = "dashed", color = "grey50") +
  scale_x_date(date_breaks = "3 months",  date_labels = "%b", limits = as.Date(c('2019-08-1', NA))) +
  scale_y_continuous(limits = c(NA,1200)) +
  theme_classic() +
  ...
  labs(y = expression("ASV richness (Chao1)"),
       x = NULL) +
...
ggplot(aes(x=date, y=Shannon)) +
  ...
  theme(#legend.text = element_markdown(),
        ...
        panel.border = element_rect(colour = "black", fill = NA)) +
  facet_wrap(~ filter_type) +
  labs(y = expression("Shannon diversity"),
       x = NULL) +
  theme(axis.text.x = element_text(hjust = 0.5))
...
# save plot
ggsave(prow, filename = "figures/plot_alpha_div_OBER_rare13092_granules.pdf", width = 11, height = 4)
...
```{r plot avg beta diversity ASV timepath granules, eval=T, echo=F, fig.height=3, fig.width=, message=FALSE, warning=FALSE, cache=FALSE, include=T}
...
p_before = 
  ...
  filter(sampling_time == "before_backwash") %>% 
  ...
  scale_color_gradient(low = "#0095FF", high = "#00457A", guide = "none") +
...
p_after = 
  plot_beta_tbl_granules %>% 
  filter(sampling_time == "after_backwash") %>% 
  dplyr::mutate(sampling_date = ymd(sampling_date),
         sampling_time = factor(sampling_time, levels = c("before_backwash", "after_backwash")),
         month = month.abb[month],
         month_year = paste0(month, " ", year)) %>% 
  ...
  geom_path(aes(color = sampling_date), size = 1.5) +
  #scale_color_distiller(guide = "none") +
  scale_color_gradient(low = "#FF6AFF", high = "#A300A3", guide = "none") +
  scale_y_continuous(expand = c(-0.3, 0.4)) +
  facet_grid(cols=vars(filter_type)) +
  labs(x = labs[1], y=labs[2]) +
  theme_classic() +
...
p_before = p_before +  geom_text_repel(aes(label = month_year, color = sampling_date))
p_after = p_after +  geom_text_repel(aes(label = month_year, color = sampling_date))

plot_beta_granules_grid = 
  plot_grid(p_before + theme(legend.position = "none"),
...
# save plot
ggsave(plot = plot_beta_granules_grid, 
       file = stringr::str_glue("figures/{proj}_beta_div_PCoA_BrayCurtis_backwash_filtertype.pdf"), 
       width = 6, 
       height = 7)
...
OBER_rare21227_table_backwash_long_ASV %>% 
  ...
  filter(tax_label %in% top10_ASVs_backwash,
         rel_abund > 0.0099) %>% 
  write.csv2(., file = "output_data/data_bubbleplot.csv", col.names = T, na = "")

# average 'core' abundance across all granule samples with positive detection
# average core taxa abundance across B1 and B2 and before and after backwashing
OBER_rare13092_table_granules_long_ASV %>% 
  #select(sampleID, ASV, rel_abund) %>%
  inner_join(., tax_beta_div %>% dplyr::rename(ASV = "taxon"), by = "ASV") %>% 
  dplyr::mutate(tax_label = paste0(Class, "__", Order, "__",Family, "__", Genus)) %>% 
  ...
  inner_join(., top10_ASVs_granules_classes %>% select(tax_label, tax_name, functional_group), by = "tax_label") %>% 
  dplyr::mutate(sampling_time = factor(sampling_time, levels = c("before_backwash","after_backwash"))) %>% 
  group_by(tax_label, sampling_time, filter_type) %>% 
  summarise(mean_pct = mean(rel_abund*100), stdev = sd(rel_abund*100), n_samples = n()) %>% 
  arrange(desc(mean_pct), .by_group = F) %>% 
  ungroup() %>% 
  mutate(across(where(is.numeric), round, digits = 3)) %>% 
  #pivot_wider(names_from = filter_type, values_from = c(mean_pct, stdev, n_samples)) %>% 
  pivot_wider(names_from = sampling_time, values_from = c(mean_pct:n_samples)) %>% 
  write.csv2(., file = "output_data/data_bubbleplot_mean_rel_abund_overall.csv", col.names = T, na = "", dec = ".")


core90_granules_abund %>%  
  dplyr::rename(ASV = "OTU") %>% 
  dplyr::mutate(tax_label = paste0(Class, "__", Order, "__",Family, "__", Genus)) %>% 
  select(sampleid, tax_label, Abundance) %>%
  group_by(sampleid, tax_label) %>% 
  ...
  inner_join(., 
             tax_beta_div %>% 
               ...
               dplyr::mutate(tax_label = paste0(Class, "__", Order, "__",Family, "__", Genus)),
             by = "tax_label") %>% 
  group_by(tax_label, ASV, sampleid) %>% 
  summarise(rel_abund = sum(rel_abund)) %>% 
  # filter(#tax_label %in% top10_ASVs_granules,
  #        rel_abund > 0.0099) %>% 
  inner_join(., core90_granules_abund %>% select(ASV = "OTU", sampleid, sampling_time, filter_type, sampling_date), by = "sampleid") %>% 
  dplyr::mutate(sampling_time = factor(sampling_time, levels = c("before_backwash","after_backwash")),
                date = ymd(sampling_date)) %>% 
  group_by(tax_label, sampleid, sampling_time, filter_type, date) %>% 
  summarise(rel_abund = sum(rel_abund)) %>% 
  ggplot(aes(x=date, y=tax_label)) +
# geom_point(aes(size = rel_abund, alpha = rel_abund, color = rel_abund)) +
  geom_tile(aes(alpha = rel_abund, fill = rel_abund)) +
  scale_size_continuous(breaks = c(0.01, 0.05, 0.1, 0.2, 0.3),
                        labels = c("1%","5%","10%", "20%", "30%")) +
