---
title: "OBER_Proj2_BODAC_seasonality_part2"
...
omp_data = 
  read.table("raw_data/OBER_16S_515F926R_Q14878_SAM1-52@metadata_omp.csv",
                      header = T, sep = ";") %>% 
  mutate(sampling_date = ymd(sampling_date)) %>% 
  mutate_if(is.character, as.factor) %>% 
  mutate(sample_source = factor(sample_source, 
                                levels = c("UF_permeate", "BODAC_1_effluent", "BODAC_2_effluent")),
         timing = factor(timing, levels = c("before_backwash", "after_backwash"))) %>% 
  select(-sd) %>% 
  # pivot_wider(names_from = "analysis", values_from = "mean", values_fill = NA_real_) %>% head
  # filter(timing == "before_backwash") %>% 
  drop_na(mean) 

bulk_data = 
  read.table("raw_data/OBER_16S_515F926R_Q14878_SAM1-52@metadata_bulk.csv",
                      header = T, sep = ";") %>% 
  mutate(sampling_date = ymd(sampling_date)) %>% 
  mutate_if(is.character, as.factor) %>% 
  select(-sd) %>% 
  pivot_wider(names_from = "analysis", values_from = "mean")
```
### Explore metadata
...
omp_data %>% 
  ggplot(aes(x=sampling_date, color = analysis)) +
  #geom_point(aes(y=mean)) +
  geom_line(aes(y=mean)) +
  #facet_wrap(~sample_source) +
  facet_grid(rows = vars(timing), cols = vars(sample_source)) +
  ...
  labs(y = "Concentration (unit, log-scale)",
       x = "Time (date)")
...
```{r calculate avg beta diversity ASV granules and backwash, eval=T, echo=F, fig.height=3, fig.width=8, message=FALSE, warning=FALSE, cache=FALSE, include=T}

# reshape the time variable from dates to days (januari days from 2019 onwards)
sample_data(psdata_OBER_beta_granules)$date = lubridate::ymd(sample_data(psdata_OBER_beta_granules)$sampling_date)
sample_data(psdata_OBER_beta_backwash)$date = lubridate::ymd(sample_data(psdata_OBER_beta_backwash)$sampling_date)

sample_data(psdata_OBER_beta_granules)$days = lubridate::yday(sample_data(psdata_OBER_beta_granules)$date)
sample_data(psdata_OBER_beta_backwash)$days = lubridate::yday(sample_data(psdata_OBER_beta_backwash)$date)

sample_data(psdata_OBER_beta_granules)$days = if_else(sample_data(psdata_OBER_beta_granules)$year == 2019, sample_data(psdata_OBER_beta_granules)$days, if_else(sample_data(psdata_OBER_beta_granules)$year == 2020, sample_data(psdata_OBER_beta_granules)$days + 365, sample_data(psdata_OBER_beta_granules)$days + 365 + 366))

sample_data(psdata_OBER_beta_backwash)$days = if_else(sample_data(psdata_OBER_beta_backwash)$year == 2019, sample_data(psdata_OBER_beta_backwash)$days, if_else(sample_data(psdata_OBER_beta_backwash)$year == 2020, sample_data(psdata_OBER_beta_backwash)$days + 365, sample_data(psdata_OBER_beta_backwash)$days + 365 + 366))

# sample data
sam_data_granules <- data.frame(sample_data(psdata_OBER_beta_granules))
sam_data_backwash <- data.frame(sample_data(psdata_OBER_beta_backwash))

# calculate adonis permanova on granules only on ASV level Bray-Curtis diss

# granules adonis
adonis_bc_granules = adonis2(OBER_rare13092_granules ~ filter_type + days + sampling_time, permutations = 999, data = sam_data_granules) # R crashes by using the by = 'margin' argument


adonis_bc_granules %>% broom::tidy() %>% kableExtra::kbl(caption = "Granules: Bray-Curtis dissimilarity") %>% kableExtra::kable_minimal() %>% 
kableExtra::save_kable("output_data/Table_Adonis_granules_BC_rare13092.jpg", bs_theme = "simplex", self_contained = TRUE)


# backwash water adonis
psdata_OBER_beta_backwash

adonis_bc_backwash = vegan::adonis2(OBER_rare21227_backwash ~ filter_type + days, permutations = 999, data = sam_data_backwash) # R crashes by using the by = 'margin' argument

adonis_bc_backwash %>% broom::tidy() %>% kableExtra::kbl(caption = "Backwash water: Bray-Curtis dissimilarity") %>% kableExtra::kable_minimal() %>% 
kableExtra::save_kable("output_data/Table_Adonis_backwash_BC_rare21227.jpg", bs_theme = "simplex", self_contained = TRUE,)
...
```{r plot avg beta diversity ASV granules, eval=F, echo=F, fig.height=3, fig.width=8, message=FALSE, warning=FALSE, cache=FALSE, include=F}
...
plot_beta_tbl_granules <- positions_granules %>%  
  as_tibble(rownames = "samples") %>% 
  inner_join(., metadata_cleaned, by = c("samples"="sampleid")) %>% 
  mutate(combi = factor(paste0(filter_type, "_", sampling_time), levels = c("BODAC_1_before_backwash", "BODAC_1_after_backwash",  "BODAC_2_before_backwash", "BODAC_2_after_backwash"))) 
...
OBER_rare13092_table_granules %>% 
  ...
  rowwise() %>% mutate(sample_sums = sum(c_across(2:ncol(.)))) %>% 
...
OBER_rare13092_table_granules_long_ASV %>% 
  ...
  mutate(tax_label = paste0(Class, "__", Order, "__",Family, "__", Genus)) %>% 
...
top10_ASVs_granules_classes = read.table("output_data/top10_ASVs_granules_classified.csv", header = T, sep = ";") %>% 
  as_tibble() %>% 
  mutate(functional_group = factor(functional_group, levels = c("AOA", "AOB", "put_AOB", "NOB_Commamox", "MnOB", "put_MnOB", "others")))
...
OBER_rare13092_table_granules_long_ASV %>% 
  ...
  mutate(sampling_time = factor(sampling_time, levels = c("before_backwash","after_backwash"))) %>% 
  ggplot(aes(x=date, y=tax_name)) +
  scale_size_continuous(breaks = c(0.01, 0.05, 0.1, 0.2, 0.3),
                        labels = c("1%","5%","10%", "20%", "30%")) +
  scale_color_gradient(low = "grey60", high = "grey10") +
...
ggsave(plot = top_asv_rel_abund_granules, 
       file = stringr::str_glue("figures/{proj}_ASV_top10_rel_abund_granules_grey.pdf"), 
       width = 10, 
       height = 7)
...
OBER_rare13092_table_granules_long_ASV %>% 
  ...
  mutate(sampling_time = factor(sampling_time, levels = c("before_backwash","after_backwash"))) %>% 
  ungroup() %>% 
  dplyr::select(sampleID, tax_label, rel_abund) %>% 
  pivot_wider(names_from = sampleID, values_from = rel_abund, values_fill = 0) %>% 
  write_csv(., glue::glue("output_data/{proj}_Bubbleplot_data.csv"))


```

### TODO: repeat / redo granule bubble plot with classifications also for backwash water
